...
boolean hooterHasFired = false; // <--- ADDED: This locks the sound forever after 1 play
...
void launchPython() {
  ...
  String[] command = {"cmd", "/c", "start", "cmd", "/c", runCmd}; // Changed /k to /c for better experience
...
void drawText() {
  ...
  if (iDistance > 0 && iDistance <= maxDistance) {
    ...
    // This handles the one-time beep
    if (hooter != null && !hooterHasFired) {
      hooter.play();           // Play once (not loop)
      hooterHasFired = true;   // This prevents it from ever running again
      println("🔊 Hooter beeped once. Locked for this session.");
    }
  } else {
    text("Out of Range", 250, height - 20);
    // We do NOT reset hooterHasFired here, so it stays locked.
  }
}
