...
#  AUTO-SCAN: Camera starts immediately on launch
#  FILE INPUT: Supports loading Image / Video files
#  Detects objects via YOLOv8 ONNX:
...
COCO_NAMES = [
    ...
    "toaster","sink","refrigerator","book","clock","vase","scissors","knife","teddy bear",
...
class VisionSystem:
    def __init__(self, root: tk.Tk):
        ...
        self._file_mode = None   # None = camera, "image" = static image, "video" = video file
        self._file_frame = None  # holds the loaded image frame (image mode)

        # watcher thread (still watches trigger file for external triggers)
        self._watcher_thread = threading.Thread(
            target=self._watch_trigger, daemon=True
        )
        self._watcher_thread.start()

        # AUTO-START: begin scanning immediately without any button click
    ...
    def _build_ui(self):
        ...
        tk.Label(
            info_frame,
            text="Camera starts automatically",
            font=("Consolas", 8),
            fg="#446644",
            bg="#0d1117",
        ).pack(pady=(0, 6))
        ...
        # ── Turn on Camera Button (shown after file input) ───────────
        ttk.Separator(info_frame, orient="horizontal").pack(fill=tk.X, padx=8, pady=4)

        self._cam_btn_frame = tk.Frame(info_frame, bg="#0d1117")
        self._cam_btn_frame.pack(fill=tk.X, padx=10, pady=(4, 2))

        self._cam_on_btn = tk.Button(
            self._cam_btn_frame,
            text="📷  TURN ON CAMERA",
            font=("Consolas", 11, "bold"),
            bg="#003322",
        ...
        self._cam_on_btn.pack(fill=tk.X)

        # Pulse animation label under the camera button
        self._cam_hint_lbl = tk.Label(
            self._cam_btn_frame,
            text="↑ Click to return to live feed",
            font=("Consolas", 8),
            fg="#226644",
            bg="#0d1117",
        )
        self._cam_hint_lbl.pack(pady=(2, 4))

        # Hide camera button initially (camera is auto-started)
        self._cam_btn_frame.pack_forget()
    ...
    def _start_camera(self):
        """Start live camera feed (always resets file mode)."""
        self._stop_camera()          # cleanly stop whatever is running
        self._file_mode = None
        self._file_frame = None
        self.running = True
        self.status_lbl.config(text="● SCANNING", fg="#00ff60")
        # Hide the "Turn on Camera" button — we are now live
        self._cam_btn_frame.pack_forget()
    ...
    def _open_image(self):
        """Let the user pick an image file and run detection on it once."""
        ...
        self._stop_camera()
        self._file_mode = "image"
        self._file_frame = frame.copy()

        self.status_lbl.config(text="● IMAGE MODE", fg="#ffcc00")
        frame_out, detections = self._detect(self._file_frame.copy())
        ...
        if detections:
            ...
            self._log("[IMG] No objects detected.")

        # Show the "Turn on Camera" button so user can return to live feed
        self._show_camera_button()

    # -------------------------------------------------------------- #
    def _open_video(self):
        """Let the user pick a video file and stream it with live detection."""
        path = filedialog.askopenfilename(
        ...
        self._stop_camera()
        self._file_mode = "video"
        self.cap = cap
        self.running = True
        self.status_lbl.config(text="● VIDEO MODE", fg="#6699ff")
        # Show the "Turn on Camera" button so user can switch back anytime
        self._show_camera_button()
        self._update_frame()   # reuse the same frame-loop

    # -------------------------------------------------------------- #
    def _stop_camera(self):
        """Stop camera / video playback and show a blank frame."""
        self.running = False
        self.trigger_active = False
    ...
    # -------------------------------------------------------------- #
    def _show_camera_button(self):
        """Make the 'Turn on Camera' button visible with a pulse animation."""
        # Pack the button frame (insert it after the File Input separator)
        self._cam_btn_frame.pack(fill=tk.X, padx=10, pady=(4, 2))
        # Trigger a green pulse on the button to draw attention
        self._pulse_camera_btn(0)

    # -------------------------------------------------------------- #
    def _pulse_camera_btn(self, step: int):
        """Animate the camera button with alternating glow to catch the user's eye."""
        colors = [
            ("#003322", "#00ff99"),  # dark
            ("#005533", "#aaffdd"),  # bright
        ]
        bg, fg = colors[step % 2]
        try:
            self._cam_on_btn.config(bg=bg, fg=fg)
            # Keep pulsing if camera is still off (file mode or standby)
            if not self.running:
                self.root.after(700, lambda: self._pulse_camera_btn(step + 1))
            else:
                # Camera is running — restore normal color and hide
                self._cam_on_btn.config(bg="#003322", fg="#00ff99")
        except Exception:
            pass

    # -------------------------------------------------------------- #
    def _log(self, text: str):
        """Append a plain text entry to the detection log box."""
    ...
    def _update_frame(self):
        ...
        if not ret:
            ...
            if self._file_mode == "video":
                self._stop_camera()
                self.status_lbl.config(text="● VIDEO ENDED", fg="#ffcc00")
                self._log("[VIDEO] Playback finished.")
                # Video finished — prompt user to turn camera back on
                self._show_camera_button()
            else:
        ...
        # Video plays slightly slower than camera to avoid skipping frames
        delay = 30 if self._file_mode == "video" else 15
